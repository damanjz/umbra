...
files.forEach(file => {
    ...
    if (articles.length > 1) {
        for (let i = 1; i < articles.length; i++) {
            PRODUCTS.forEach(p => {
                // Check if this article is for product p
                if (articles[i].includes(`>${p.name}<`) || articles[i].includes(`>${p.name}</a><`)) {
                    changed = true;
                    // 1. Link the Title
                    const regexTitle = new RegExp(`<h3 class="product-card__name">(<a[^>]*>)?${p.name.replace(/[.*+?^$\\{\\}()|[\\]\\\\]/g, '\\\\$&')}(<\\/a>)?<\\/h3>`);
                    articles[i] = articles[i].replace(regexTitle, `<h3 class="product-card__name"><a href="products/${p.id}.html">${p.name}</a></h3>`);

                    // 2. Link the Image
                    articles[i] = articles[i].replace(/<div class="product-card__img">(.*?)<\/div>/, `<a href="products/${p.id}.html" class="product-card__img">$1</a>`);

                    // 3. Update Image SRC
                    articles[i] = articles[i].replace(/<img src="images\/[^"]+"/, `<img src="images/${p.image}"`);

                    // 4. Update onclick param
                    articles[i] = articles[i].replace(/onclick="addToCart\('[^']+',\d+,'images\/[^']+'\)"/, `onclick="addToCart('${p.name.replace(/'/g, "\\'")}',${p.price},'images/${p.image}')"`);
                }
            });
        }
